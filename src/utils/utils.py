...
os.makedirs("logs/logs_utils", exist_ok=True)
os.makedirs("logs/logs_errors/logs_errors_utils", exist_ok=True)

log_formatter = logging.Formatter("%(asctime)s - %(levelname)s - %(message)s")
log_handler = logging.FileHandler(f"logs/logs_utils/log_utils_{date_str}.log")
log_handler.setFormatter(log_formatter)

error_handler = logging.FileHandler(f"logs/logs_errors/logs_errors_utils/logs_errors_utils_{date_str}.log")
...
def detect_lines(roi: np.ndarray) -> bool:
    ...
    try:
        ...
        lines = cv2.HoughLinesP(edges, 1, np.pi/180, threshold=85, minLineLength=60, maxLineGap=10)
